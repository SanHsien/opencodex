...
// Fork overlay: only the English and Traditional Chinese catalogs are kept (docs/fork/UPSTREAM.md).
...
describe("the dashboard recovery roster cannot drift from the durable one", () => {
  ...
  test("every label key the page names exists in every kept catalog", () => {
    const page = readFileSync(repoPath("gui", "src", "pages", "Logs.tsx"), "utf8");
