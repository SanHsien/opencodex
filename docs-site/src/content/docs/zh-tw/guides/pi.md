...
模型 id 是 proxy 的規範選擇器，因此路由模型會顯示為 `provider/model`
（`anthropic/claude-opus-5`），而原生 OpenAI slug 維持無前綴（`gpt-5.6-sol`）。`name`
的字尾——`(anthropic)`、`(native)`、`(routed)`——正是讓來自不同上游的兩個同名模型在 Pi 的
選單中可以區分的關鍵。
